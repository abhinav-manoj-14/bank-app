import styles from "../style";
import { discount, robot } from "../assets";
import GetStarted from "./GetStarted";

const Hero = () => {
   return (
      <section id="home" className={`flex md:flex-row flex-col ${styles.paddingY}`}>
         {/* The left hand part of the home page, that is the text part excluding the drawing */}
         <div className={`flex-1 ${styles.flexStart} flex-col xl:px-0 sm:px-16 px-6`}>
            {/* The first small text box of discount */}
            <div className={`flex flex-row items-center bg-discount-gradient py-[6px] px-4 rounded-[10px] mb-2`}>
               <img src={discount} alt="discount" className="w-[32px] h-32[px]" />

               <p className={`${styles.paragraph} ml-2`}>
                  <span className="text-white">20% </span>
                  Discount For <span className="text-white">1 Month </span>
                  Account
               </p>
            </div>

            {/* The next part which is the big heading and the getting started logo */}
            <div className="flex flex-row justify-between items-center w-full ">
               {/* "The Next Generation" */}
               <h1
                  className="flex-1 font-poppins font-semibold ss:text-[72px] 
                  text-[52px] text-white ss:leading-[100px] leading:75[px]"
               >
                  The Next <br className="sm:block hidden" />
                  <span className="text-gradient">Generation </span>
               </h1>

               {/* The get started logo component */}
               <div className={`ss:flex md:mr-4 mr-0`}>
                  <GetStarted />
               </div>
            </div>

            {/* "Payment Method." */}
            <h1
               className="font-poppins font-semibold ss:text-[68px] 
                  text-[52px] text-white ss:leading-[100px] leading:75[px] w-full"
            >
               Payment Method.
            </h1>

            <p className={`${styles.paragraph} max-w-[483px] mt-5`}>
               Our team of experts uses a methodology to identify the credit cards most likely to fit your needs. We examine annual percentage rates,
               annual fees.
            </p>
         </div>

         <div className={`flex-1 flex ${styles.flexCenter} relative`}>
            <img src={robot} alt="robot hand" className=" w-[100%] h-[100%] relative z-[5]" />

            {/* Gradience colouring */}
            <div className="absolute w-[40%] h-[30%] top-0 pink__gradient"></div>
            <div className="absolute w-[80%] h-[80%] rounded-full bottom-40 white__gradient"></div>
            <div className="absolute w-[50%] h-[50%] right-20 bottom-20 blue__gradient"></div>
         </div>
      </section>
   );
};

export default Hero;
